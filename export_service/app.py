...
class ExportService:
    ...
    async def connect(self):
        ...
        logger.info(f"Connected to RabbitMQ, listening on queue: {RABBITMQ_EXPORT_QUEUE}")
    ...
    async def handle_message(self, message: aio_pika.IncomingMessage):
        ...
        async with message.process():
            try:
                ...
                succeeded = await asyncio.to_thread(self.handle_export, payload)
    ...
    def handle_export(self, payload: dict) -> bool:
        """Handle export logic"""
        try:
            topic_id = payload.get('topic_id')
            ...
            #     # Send to Email Service 2 asynchronously
            #     asyncio.create_task(self.send_to_email_service_2(email_payload))
            #     logger.info(f"Sent payload to Email Service 2 for {filename}")
                return True
            else:
                return False
